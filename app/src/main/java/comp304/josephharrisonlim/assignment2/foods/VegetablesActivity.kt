...
class VegetablesActivity : AbstractOptionsMenuActivity() {
    ...
    private val vegetableItems: Array<Food> = arrayOf(
            Food("Bell Peppers", 4.00, FoodType.VEGETABLES.name),
            Food("Cucumber", 0.99, FoodType.VEGETABLES.name),
            Food("Romain Lettuce", 3.00, FoodType.VEGETABLES.name)
    )

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_vegetables)
        vegetableItems.map { food ->
